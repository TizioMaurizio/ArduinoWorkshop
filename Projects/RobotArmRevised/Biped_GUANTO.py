...
poseOffset10 = [0, 12, 31, 1, -10, 14, 5, -18, -17, 0]
savedPoses = dict()
jointSliders = []
savedButtons = []
x = 45
y = (16255463 * x ** 4) / 3252654662400 - (29670149123 * x ** 3) / 3252654662400 + (
                    6719804604907 * x ** 2) / 1084218220800 - (
                                   6036634120959517 * x) / 3252654662400 + 4936954994391391 / 23916578400
glovehand = [y,y,y,y,y,y,y,y,y]

try:
    glove = serial.Serial(port='COM13', baudrate=9600)
...
def send_angles_socket():
    global glovehand, prevTime, RECEIVER, jsonPoses, servoValue, INTERVAL, STEP, prevValue, master, END, currentMotor ,prevMotor, doReset, jsonPoses, prevJsonPoses, message, positions, buttons
    import socket
    import traceback
    host = ''
    port = 5555
        ...
        try:
            ...
            if(time.time()-prevTime >= INTERVAL):
                ...
                if(jsonPoses != prevJsonPoses or startup):
                    ...
                    glovehand = [int(i) for i in str(glove.readline())[2:-6].split(" ")]
...
def allMotors():
    global jointSliders, jsonPoses, glovehand
    for i in range(10):
        if i >= 5:
            j = 15 - (9-i)
        else:
        ...
        x = glovehand[6]
        glovehand[6] = (16255463 * x ** 4) / 3252654662400 - (29670149123 * x ** 3) / 3252654662400 + (
                    6719804604907 * x ** 2) / 1084218220800 - (
                                   6036634120959517 * x) / 3252654662400 + 4936954994391391 / 23916578400
        jsonPoses[j] = startingPoses[i] + poseOffset[j]
        if j == 11 :#and (glovehand[6] < 90 and glovehand[6] > 45):
                
                jsonPoses[j] = int(glovehand[6]) + poseOffset[j] #GUANTO
