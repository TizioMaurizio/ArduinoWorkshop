...
//Motor variables
int targetPoses[16] = {90, 180, 150, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90, 90}; //from hand to swing
unsigned long previousMillis = 0;
const long INTERVAL = 20; //1 degree every 17ms (about 60 degrees per second) --NOTE one cycle seems to take around 10ms, lowering the value under that is harmful for performance
int increment = 4; //change this to speed up movement once interval is minimized
...
void loop() {
  ...
  for(int i=0; i<5; i++){
    ...
    if(val>=0 && val<=180){
     ...
     //Serial.print(" p"); Serial.print(i); Serial.print(": ");  Serial.print(val); Serial.print(" "); Serial.print(activated[i]); Serial.print("  |");
  }
  Serial.println();
